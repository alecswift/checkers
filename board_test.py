...
class TestBoard(unittest.TestCase):
    ...
    def test_valid_move_method_with_one_valid_capture(self):
        state = game.make_move((self.b_piece, (5j, 1 + 4j), ()), self.initial_state)
        state = game.make_move((self.w_piece, (3 + 2j, 2 + 3j), ()), state)
    ...
    def test_valid_move_method_with_one_valid_capture_2(self):
        state = game.make_move((self.b_piece, (2 + 5j, 3 + 4j), ()), self.initial_state)
        state = game.make_move((self.w_piece, (3 + 2j, 4 + 3j), ()), state)
        state = game.make_move((self.b_piece, (1 + 6j, 2 + 5j), ()), state)
        state = game.make_move((self.w_piece, (5 + 2j, 6 + 3j), ()), state)
        state = game.make_move((self.b_piece, (3 + 4j, 5 + 2j), (4 + 3j,)), state)
    ...
    def test_valid_move_method_with_2_valid_captures(self):
        state = game.make_move((self.b_piece, (4 + 5j, 5 + 4j), ()), self.initial_state)
        state = game.make_move((self.w_piece, (5 + 2j, 6 + 3j), ()), state)
        state = game.make_move((self.b_piece, (5 + 6j, 4 + 5j), ()), state)
        state = game.make_move((self.w_piece, (3 + 2j, 4 + 3j), ()), state)
        state = game.make_move((self.b_piece, (5 + 4j, 3 + 2j), (4 + 3j,)), state)
        expected = {
    ...
    def test_valid_paths_method_with_double_capture(self):
        state = game.make_move((1, 5j, 1 + 4j), self.initial_state)
        state = game.make_move((2, 5 + 2j, 4 + 3j), state)
        state = game.make_move((1, 6 + 5j, 7 + 4j), state)
        state = game.make_move((2, 6 + 1j, 5 + 2j), state)
        state = game.make_move((1, 1 + 6j, 5j), state)
        state = game.make_move((2, 4 + 3j, 3 + 4j), state)
    ...
    def test_valid_paths_method_with_double_capture_2(self):
        ...
        state = game.make_move((2, 6 + 1j, 5 + 2j), state)
        state = game.make_move((1, 1 + 6j, 5j), state)
        state = game.make_move((2, 5 + 0j, 6 + 1j), state)
        state = game.make_move((1, 7j, 1 + 6j), state)
        state = game.make_move((2, 3 + 2j, 2 + 3j), state)
        expected = {(1, 1 + 4j, 2 + 3j, 3 + 2j, 4 + 1j, 5 + 0j)}
        self.valid_paths_helper(state, expected)

    def test_valid_paths_with_king_double_capture(self):
        state = game.make_move((1, 6 + 5j, 7 + 4j), self.initial_state)
        state = game.make_move((2, 1 + 2j, 2 + 3j), state)
        state = game.make_move((1, 5 + 6j, 6 + 5j), state)
        state = game.make_move((2, 1j, 1 + 2j), state)
        state = game.make_move((1, 4 + 7j, 5 + 6j), state)
        state = game.make_move((2, 2 + 3j, 3 + 4j), state)
        state = game.make_move((1, 4 + 5j, 3 + 4j, 2 + 3j, 1 + 2j, 1j), state)
        state = game.make_move((2, 2 + 1j, 1 + 2j), state)
        state = game.make_move((1, 5j, 1 + 4j), state)
        state = game.make_move((2, 1 + 0j, 2 + 1j), state)
        state = game.make_move((1, 1j, 1 + 0j), state)  # black piece promoted
        state = game.make_move((2, 3 + 2j, 4 + 3j), state)
